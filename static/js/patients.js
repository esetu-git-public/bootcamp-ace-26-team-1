(function () {
  ...
  document.getElementById('searchInput').addEventListener('input', () => {
    ...
    searchTimer = setTimeout(() => { currentPage = 1; loadPatients(); }, 350);
  });
  document.getElementById('prevPageBtn').addEventListener('click', () => { currentPage--; loadPatients(); });
  document.getElementById('nextPageBtn').addEventListener('click', () => { currentPage++; loadPatients(); });

  // ---- Upload CSV ----
  const uploadBtn = document.getElementById('uploadBtn');
  const uploadPanel = document.getElementById('uploadPanel');
  uploadBtn.addEventListener('click', () => {
    uploadPanel.style.display = uploadPanel.style.display === 'none' ? 'flex' : 'none';
  });

  document.getElementById('csvSubmitBtn').addEventListener('click', async () => {
    const input = document.getElementById('csvInput');
    const resultEl = document.getElementById('uploadResult');
    if (!input.files.length) { resultEl.textContent = 'Choose a CSV file first.'; return; }
    const formData = new FormData();
    formData.append('file', input.files[0]);
    resultEl.textContent = 'Uploading…';
    const res = await window.ReAdmitIQ.api('/api/patients/upload-csv', {
    ...
    const data = await res.json();
    resultEl.textContent = `Imported ${data.inserted} rows.${data.warnings.length ? ' ' + data.warnings.join(' ') : ''}`;
    currentPage = 1;
    loadPatients();
  });
